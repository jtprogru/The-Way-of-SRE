...
**L3**
- Понимает, что container — это процесс в namespaces/cgroups (см. [Operating Systems](/The-Way-of-SRE/engineering/operating-systems/)), а pod — группа контейнеров с общим network namespace. Различает `Deployment` / `StatefulSet` / `DaemonSet` / `Job` / `CronJob` и знает, для чего каждый.
...
**L5**
- Управляет workload через GitOps-flow (см. [GitOps](/The-Way-of-SRE/engineering/gitops/)): Helm/Kustomize шаблоны, разделение значений по окружениям, ArgoCD/Flux sync drift detection. На petproject'е `orb-k8s-gitops` — пример паттерна «приложение / окружение / values».
...
**L6+**
- Оценивает trade-off self-hosted vs managed (EKS / GKE / AKS / managed-k8s облака — см. [Cloud Providers](/The-Way-of-SRE/engineering/cloud-providers/)). Понимает, что отдаётся провайдеру (control plane uptime, etcd, upgrades) и что остаётся у команды (worker nodes, addons, networking, observability).
...
- Учит команду: разбирает с командой инциденты в k8s как демонстрацию, ведёт планы апгрейдов, документирует cluster invariants (что нельзя сломать) в [runbook'ах](/The-Way-of-SRE/culture/runbooks/).
...
- Liz Rice — **[Container Security](https://www.oreilly.com/library/view/container-security/9781492056690/)** (O'Reilly, 2020). Уже упомянута в [Operating Systems](/The-Way-of-SRE/engineering/operating-systems/); тут особенно главы про container runtime и k8s security model.
...
- **[ArgoCD](https://argo-cd.readthedocs.io/)** / **[Flux](https://fluxcd.io/)** — операторы GitOps (см. [GitOps](/The-Way-of-SRE/engineering/gitops/)). Использую ArgoCD на [`evo-tf-argocd`](https://github.com/jtprogru/evo-tf-argocd) — демо для Cloud.ru Evolution Managed Kubernetes.
...
- **[Operating Systems](/The-Way-of-SRE/engineering/operating-systems/)** — container = процесс в namespaces/cgroups; дебаг pod часто заканчивается на kernel-level (`/proc`, `nsenter`, `dmesg`).
- **[Networking](/The-Way-of-SRE/engineering/networking/)** — оверлейные сети (Calico, Cilium, Flannel), CNI plugin internals, NetworkPolicy, kube-proxy / IPVS, ingress controllers — стык с сетевым доменом.
- **[Service Mesh](/The-Way-of-SRE/engineering/service-mesh/)** — следующий слой поверх pod-to-pod трафика: mTLS, traffic shifting, L7 observability через sidecar.
- **[Cloud Providers](/The-Way-of-SRE/engineering/cloud-providers/)** — managed-k8s (EKS / GKE / AKS / Yandex Managed Kubernetes) меняет shared responsibility model; control plane живёт у провайдера.
- **[Infrastructure as Code](/The-Way-of-SRE/engineering/infrastructure-as-code/)** — кластер провижится через Terraform / Pulumi; манифесты — следующий слой того же декларативного подхода.
- **[GitOps](/The-Way-of-SRE/engineering/gitops/)** — Argo CD / Flux синхронизируют git с состоянием кластера; естественный механизм доставки для workload'ов, живущих в k8s.
- **[Resilience Patterns](/The-Way-of-SRE/engineering/resilience-patterns/)** — PDB (PodDisruptionBudget), HPA (HorizontalPodAutoscaler), topology spread constraints — k8s-native реализации resilience patterns.
- **[Capacity Planning](/The-Way-of-SRE/engineering/capacity-planning/)** — k8s requests/limits + cluster autoscaling переводят планирование capacity в декларативный формат.
