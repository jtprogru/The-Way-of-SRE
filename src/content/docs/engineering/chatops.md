...
- **[Toil Automation](/The-Way-of-SRE/engineering/toil-automation/)** — parent: ChatOps — chat-driven форма automation; естественное продолжение notification automation в сторону bot-driven actions.
- **[Toil Tracking](/The-Way-of-SRE/engineering/toil-tracking/)** — tracking показывает, какие повторяющиеся операционные задачи переносить в ChatOps; chat-uplift highest-frequency requests.
- **[Personal SRE Toolkit](/The-Way-of-SRE/engineering/personal-sre-toolkit/)** — personal scripts эволюционируют в team bots, когда повторяющаяся задача переходит из «моя» в «команды».
- **[Incident Response](/The-Way-of-SRE/practices/incident-response/)** — современные incident-tools (incident.io, Dispatch) — Slack-native ChatOps; declare/coordinate инцидент через chat.
- **[Severity Classification](/The-Way-of-SRE/practices/severity-classification/)** — declare-incident через `/incident sev1 <description>` — каноническая ChatOps команда.
- **[On-Call Rotation](/The-Way-of-SRE/practices/on-call-rotation/)** — `/oncall` query, escalation через bot — standard ChatOps use cases.
- **[War Room Patterns](/The-Way-of-SRE/practices/war-room-patterns/)** — chat как war room canvas; ChatOps bots координируют sitrep cadence / role rotation / scribe.
- **[Access Control & IAM](/The-Way-of-SRE/practices/access-control-iam/)** — bot identity и permission model — критичная часть ChatOps security.
- **[Workload Identity](/The-Way-of-SRE/practices/workload-identity/)** — bot — это workload; identity-based auth для bot предпочтительнее long-lived shared tokens.
