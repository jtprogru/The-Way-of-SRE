...
- **[Networking](/The-Way-of-SRE/engineering/networking/)** — реализация retries / circuit breaker / timeouts происходит в коде; знание сетевых библиотек языка — половина практики устойчивости.
- **[SLI-based Alerting](/The-Way-of-SRE/engineering/sli-based-alerting/)** — observability hooks (Prometheus client, OpenTelemetry SDK) пишутся в коде; качество SLI-метрик упирается в идиомы языка.
- **[SLO Engineering](/The-Way-of-SRE/engineering/slo-engineering/)** — graceful shutdown, idempotency, retries — кодовая основа того, что измеряется SLO.
- **[Runbooks](/The-Way-of-SRE/culture/runbooks/)** — скрипты в runbook (одноразовые скрипты-починки, диагностические выгрузки) — отдельный класс кода; bash здесь часто выигрывает у Go простотой обновления.
- **[Incident Response](/The-Way-of-SRE/practices/incident-response/)** — чтение трейсов, exception stack, pprof в инциденте — главный live-use языка.
- **[Shell & CLI Craft](/The-Way-of-SRE/engineering/shell-cli-craft/)** — соседний лист про композицию unix-tools; shell — не «приложение к Go», а отдельный muscle.
- **[Performance & Profiling](/The-Way-of-SRE/engineering/performance-profiling/)** — pprof / flame graphs / latency budgets — соседний лист про measure-first дисциплину; работает поверх runtime языка.
- **[Operating Systems](/The-Way-of-SRE/engineering/operating-systems/)** — runtime языка живёт поверх OS; GC pauses, scheduler interactions, syscall patterns — intersection.
