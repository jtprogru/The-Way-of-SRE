...
- **[Service Ownership](/The-Way-of-SRE/culture/service-ownership/)** — service catalog содержит upstream vendor dependencies; ownership of vendor relationship — explicit.
- **[Incident Response](/The-Way-of-SRE/practices/incident-response/)** — vendor incident — отдельный класс incident; IC immediately checks vendor status в первые 5 минут.
- **[Resilience Patterns](/The-Way-of-SRE/engineering/resilience-patterns/)** — graceful degradation для vendor unavailability: cached responses, fallback providers, degraded mode.
- **[Architecture Decision Records](/The-Way-of-SRE/practices/architecture-decision-records/)** — vendor selection — типичный ADR; концентрация vs diversification — recurring decision.
- **[Composite SLO Methodology](/The-Way-of-SRE/engineering/composite-slo-methodology/)** — vendor SLAs — input в composite math; этот лист ведёт inventory, composite methodology — использует.
