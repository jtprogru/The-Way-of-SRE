...
<section class="home-section">
  ...
  <div class="section-lede">
    Если хочется глубже — мотивация, методологический каркас и рабочие
    inventory-документы. Не обязательны для использования карты, но полезны
    при контрибуции и для понимания, как принимаются решения.
  </div>

  <CardGrid>
    <LinkCard
      title="О проекте"
      description="Мотивация, формат, дисклеймер. Что это и зачем — на одной странице."
      href={`${base}/about/`}
    />
    <LinkCard
      title="Методология"
      description="Принцип разделения ветвей, политика контроля детализации, оси priority и SFIA, источники структуры графа."
      href={`${base}/methodology/`}
    />
    <LinkCard
      title="Inventory: пересечения"
      description="Все известные дубли L1/L2 между ветвями и принятые решения (перенос / разделение / cross-link)."
      href={`${base}/inventory/overlaps/`}
    />
    <LinkCard
      title="Inventory: tlroadmap.io"
      description="Разбор соседнего русскоязычного проекта-роадмапа: что у них берём, что не берём, с обоснованием."
      href={`${base}/inventory/tlroadmap-review/`}
    />
  </CardGrid>
</section>
