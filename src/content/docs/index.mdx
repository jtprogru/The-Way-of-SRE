...
template: splash
hero:
  tagline: Карта направлений в SRE — три ветви, 24 компетенции верхнего уровня, листья с конкретными умениями, материалами и best practices.
---
...
// Одна фраза про ветвь для строки ниже. Порядок, имена и адреса ветвей
// берутся из roadmap.ts — руками здесь остаётся только текст.
export const branchGist = {
  culture: 'Люди, нормы, постмортем-культура.',
  engineering: 'Observability, SLO, IaC, базы данных.',
  practices: 'Incident, change, on-call, security.',
};
...
<ul class="branches-strip">
  {roadmap.branches.map((b) => (
    <li data-branch={b.id}>
      <a href={`${base}${b.href}`}>{b.label}</a>
      <span>{branchGist[b.id]}</span>
    </li>
  ))}
</ul>
