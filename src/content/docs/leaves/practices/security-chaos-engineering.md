...
- **[Security Code Review](/The-Way-of-SRE/leaves/practices/security-code-review/)** — SCR проверяет код на дефекты до деплоя (proactive, pre-merge); SCE проверяет работающие контролы в проде (validation, post-deploy). Соседи под Information Security.
