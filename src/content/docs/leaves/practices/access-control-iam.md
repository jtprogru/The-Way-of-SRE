...
- **[Vulnerability Management](/The-Way-of-SRE/leaves/practices/vulnerability-management/)** — IAM сам по себе — vulnerability surface (excessive permissions, orphan accounts); VM tools часто scan'ят IAM misconfigurations.
...
Федерация workload identity в multi-cloud частично закроется в [Workload Identity](/The-Way-of-SRE/leaves/practices/workload-identity/), но нюансы вроде cross-cloud trust и проверки OIDC issuer туда пока не помещаются. Отдельно интересна объяснимость доступа — вопрос «почему пользователь X получил доступ к ресурсу Y» в разросшейся модели. Платформы ReBAC на него отвечают, в RBAC и ABAC ответ приходится собирать руками.
