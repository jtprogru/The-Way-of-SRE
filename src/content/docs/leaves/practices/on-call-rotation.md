...
«On-call входит в зарплату» — позиция, которую я регулярно слышу от компаний без явной on-call compensation, и одна из самых сильных причин churn senior engineers. Дежурство — это работа в нерабочее время; без явной компенсации (деньги / часы / day off) ratio of senior engineers, оставляющих команду, растёт линейно, а оставшиеся либо выгорают, либо начинают халатно реагировать. On-Call Rotation — это **дисциплина баланса** между coverage (всегда есть on-call), fairness (нагрузка распределена), sustainability (без burnout) и effectiveness (engineer на дежурстве готов реагировать). Без здоровой ротации даже идеальный incident response разваливается под burnout одного-двух людей.

## Что должен уметь

Главный навык на уровне L5 — **поддерживать on-call health** как метрику, а не «когда заметим». Sleep disruption rate, comp pay totals, fair load distribution — это объективные сигналы, которые видны до того, как engineer уходит. «No hero» culture (любой pager должен быть ack'нут, не «я разобрался без alert'а») — это часть этой дисциплины: без visibility one engineer становится bottleneck, и команда не видит проблем до его ухода.
...
- **On-call compensation обязателен — деньги, часы или day off.** Без явной компенсации resentful drift среди senior engineers; точный механизм — детали, но **факт явной компенсации** — обязательное условие здоровой ротации.
...
**«No hero» culture: любой pager должен быть ack'нут.** «Я заметил проблему до того, как alert пришёл, разобрался сам» лишает команду visibility, нарушает журнал аудита, и через 6 месяцев один engineer становится bottleneck. Это политическая позиция, которую защищает team lead — без неё team health деградирует незаметно для всех, кроме hero.
