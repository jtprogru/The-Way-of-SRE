...
- Понимает модель квот: квота — это hard limit, а не «справочная информация». На petproject [`yc-quotas-exporter`](https://github.com/jtprogru/yc-quotas-exporter) — типичная operational задача: вытаскивать квоты как метрику для Prometheus и алертить на «осталось <20%».
- Различает региональную / zonal архитектуру: что такое AZ, как сервисы себя ведут при zone down, что значит «multi-AZ» для конкретного сервиса конкретного провайдера. Знает, что разные сервисы у одного провайдера имеют разные failure isolation модели.
...
- Sam Newman — **[Building Microservices](https://www.oreilly.com/library/view/building-microservices-2nd/9781492034018/)** (O'Reilly, 2-е изд., 2021). Не cloud-specific, но главы про распределённые системы с фокусом на failure modes — фундамент для архитектуры в облаке.
...
- Yandex Cloud — **[Архитектурные практики](https://yandex.cloud/ru/docs/overview/concepts/architecture-practices)**. Аналог Well-Architected для стека Yandex Cloud. Меньше материала, чем у hyperscaler'ов, но русскоязычный и cloud-specific.
...
- **CLI провайдера** — `aws`, `gcloud`, `az`, `yc`. По моим наблюдениям, разница между SRE, который свободно живёт в облаке, и тем, кто нет — это привычка делать operations через CLI / API, а не Web UI. Web UI хорош для exploration; для repeatable operations — CLI / IaC.
- **[`yccli`](https://github.com/jtprogru/yccli)** — плагин для zsh с алиасами к `yc`. Покрывает iam, vpc, compute, managed-k8s, lockbox, KMS, managed DBs. Использую сам как примеры того, как сократить «cognitive overhead» от длинных CLI-команд.
- **[`yc-quotas-exporter`](https://github.com/jtprogru/yc-quotas-exporter)** — экспортер квот Yandex Cloud для Prometheus. Образец паттерна «quota-as-SLI»: тащим квоты как метрику, алертим на пороге.
...
- **[CloudFormation](https://aws.amazon.com/cloudformation/)** (AWS) / **[Deployment Manager](https://cloud.google.com/deployment-manager)** (GCP) — нативные IaC от провайдеров. По моим наблюдениям, чаще выбирают Terraform — потому что он мультиоблачный и вокруг него большое community, нативные tools — когда нужны provider-specific фичи.
...
- **[Infrastructure as Code](/The-Way-of-SRE/leaves/engineering/infrastructure-as-code/)** — Terraform / Pulumi — primary способ управления облачными ресурсами; CLI/UI — для exploration, не для repeatable ops.
- **[Containerization & Orchestration](/The-Way-of-SRE/leaves/engineering/container-orchestration/)** — managed Kubernetes (EKS / GKE / AKS / Yandex Managed Kubernetes) — частный случай сервиса под управлением провайдера со своей границей ответственности.
- **[Networking](/The-Way-of-SRE/leaves/engineering/networking/)** — VPC / subnet / load balancer / VPC peering — специфичные для облака network primitives; в каждом провайдере свои абстракции.
...
**Yandex Cloud как primary стек** для русскоязычных команд. Личный опыт у меня преимущественно там, отсюда и примеры. Концепты переносимы, а вот конкретика — имена сервисов, лимиты, поведение — у каждого провайдера своя. Если у вас опыт в AWS, GCP или Azure и есть желание расширить лист, PR приветствуется.

**Multi-cloud ради compliance** — отдельный класс задач, куда я не заходил. Некоторые регуляторы требуют отсутствия зависимости от единственного вендора, и как это выглядит в живой эксплуатации, я не знаю. Если у вас такое работает в проде, расскажите через PR.

**Cloud-native против cloud-agnostic** — это спектр. На одном конце «только Kubernetes и Postgres, чтобы в любой момент съехать», на другом — DynamoDB, S3 и Lambda везде, где можно. Где правильная точка для большинства команд, я не уверен; в моих проектах баланс был ближе к cloud-native, то есть брать сервисы провайдера там, где плата за них оправдана.
