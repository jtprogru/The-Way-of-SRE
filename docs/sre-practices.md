...
> **Что здесь живёт:** inventory L1 + L2 концептов компетенций ветви для GitHub-читателей. L2-узлы — это потенциальные подкомпетенции, не leaf-страницы. Фактические leaf-страницы и L1 в виде структурных данных — в [`src/data/roadmap.ts`](../src/data/roadmap.ts), визуализируются на сайте через `Spider.astro` и `BranchView.astro`. При изменении L1 (переименование, добавление, удаление) синхронизировать оба источника; priorities L1 живут только в `roadmap.ts`.
